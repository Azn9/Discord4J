...
*/
    ...
    private static int crypto_hashblocks_hl(int[] hh, int[] hl, byte[] m, final int moff, int n) {
        ...
        while (n >= 128) {
            ...
            for (i = 0; i < 80; i++) {
                ...
                h = ((ah0 >>> 28)
                        | (al0 << (32 - 28))) ^ ((al0 >>> (34 - 32))
                        | (ah0 << (32 - (34 - 32)))) ^ ((al0 >>> (39 - 32))
                        | (ah0 << (32 - (39 - 32))));
                l = ((al0 >>> 28)
                        | (ah0 << (32 - 28))) ^ ((ah0 >>> (34 - 32))
                        | (al0 << (32 - (34 - 32)))) ^ ((ah0 >>> (39 - 32))
                        | (al0 << (32 - (39 - 32))));
    ...
    public static int crypto_sign(byte[] sm, long dummy /* *smlen not used*/, byte[] m, final int moff, int/*long*/ n,
                                  byte[] sk) {
        byte[] d = new byte[64];
        byte[] h = new byte[64];
        byte[] r = new byte[64];
    ...
    public static int crypto_sign_open(byte[] m, long dummy /* *mlen not used*/, byte[] sm, final int smoff, int
            /*long*/ n, byte[] pk) {
        int i;
        byte[] t = new byte[32];
        byte[] h = new byte[64];
    ...
    public static final class Box {
        ...
        public byte[] open(byte[] box, final int boxoff, final int boxlen, byte[] theNonce) {
            if (!(box != null && box.length >= (boxoff + boxlen) && theNonce != null
